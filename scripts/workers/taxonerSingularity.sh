...
export taxoner="singularity run \
    -B $SRA_DIR:$SING_WD,$BT2_DIR:$SING_BT2,$META_DIR:$SING_BT2 \
...
#-A gets all alignment info like CIGAR style alignment score etc.
    $taxoner $THREADS \
        ...
        -A \
        --dbPath $SING_BT2 \
        --taxpath $SING_BT2/Patric_nodes.txt \
        --seq $SING_WD/trimmed/$R1 \
        --paired $SING_WD/trimmed/$R2 \
        --output $OUT_DIR \
        -y $SING_WD/extra_commands.txt
